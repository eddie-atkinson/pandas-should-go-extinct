...
# Taken from: https://github.com/gunnarmorling/1brc/blob/main/src/main/python/create_measurements.pys
...
def build_test_data(weather_station_names, num_rows_to_create):
    ...
    try:
        with open("./data/measurements.csv", "w") as file:
            progress = 0
    ...
    file_size = os.path.getsize("./data/measurements.csv")
    human_file_size = convert_bytes(file_size)

    weather = duckdb.sql(
        "create view weather as select * from read_csv('./data/measurements.csv', names=['station', 'measurements']);"
    )
    duckdb.sql("COPY (select * from weather) to './data/measurements.parquet';")
    print("Test data successfully written to data/measurements.csv")
